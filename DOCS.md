...
## 8. Telegram Mini App (UI-слой для бота)

FindOrigin включает **Telegram Mini App** — веб-интерфейс, который открывается внутри Telegram и даёт возможность вводить текст и видеть результаты (резюме, источники) в удобном виде.

### 8.1. URL Mini App

После деплоя на Vercel Mini App доступен по адресу:

```
https://ВАШ-ПРОЕКТ.vercel.app/mini
```

(Для локальной разработки с туннелем: `https://xxxx.ngrok-free.app/mini` или `https://xxxx.loca.lt/mini` — при этом туннель должен использовать HTTPS.)

### 8.2. Настройка в BotFather

**Вариант А — Menu Button (кнопка меню в чате с ботом)**

1. Откройте @BotFather в Telegram.
2. Отправьте команду `/setmenubutton`.
3. Выберите вашего бота.
4. Выберите тип **Configure menu button** → **Configure web app**.
5. Введите URL: `https://ВАШ-ПРОЕКТ.vercel.app/mini`
6. Введите текст кнопки, например: **«Открыть FindOrigin»**

После этого пользователи увидят кнопку меню (иконка меню внизу слева в чате с ботом), нажатие на которую откроет Mini App.

**Вариант Б — Inline-кнопка в сообщении бота**

Можно добавить в ответы бота кнопку, открывающую Mini App, используя `web_app` в `InlineKeyboardButton`. Пример (для разработчиков): `{ text: "Открыть приложение", web_app: { url: "https://..." } }`.

### 8.3. API

Mini App вызывает `POST /api/analyze` с телом `{ "text": "..." }`. Ответ: `{ summary?, sources[], error?, message? }`. Эндпоинт переиспользует ту же логику, что и webhook бота (поиск, AI-ранжирование, резюме).
